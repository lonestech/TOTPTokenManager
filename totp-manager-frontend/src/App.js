...
function App() {
  ...
  const checkAuthStatus = useCallback(async () => {
    try {
      const response = await axios.get('/api/github/auth-status');
  ...
  const addTOTP = useCallback(async () => {
    ...
    try {
      ...
      await axios.post('/api/totp', { userInfo, secret: processedSecret });
  ...
  const deleteTOTP = useCallback(async (id) => {
    try {
      await axios.delete(`/api/totp/${id}`);
  ...
  const generateToken = useCallback(async (id) => {
    try {
      const response = await axios.get(`/api/totp/${id}/generate`);
  ...
  const showQRCode = useCallback(async (record) => {
    try {
      const response = await axios.get(`/api/totp/${record.id}/export`);
  ...
  const uploadToGist = async () => {
    try {
      await axios.post('/api/github/upload', { mode: backupMode });
  ...
  const showRestoreModal = async () => {
    try {
      ...
      setBackupVersions([]);
      const response = await axios.get('/api/github/versions');
  ...
  const clearAllTOTPs = async () => {
    try {
      await axios.post('/api/totp/clear-all');
  ...
  const handleRestoreModalOk = async (gistId) => {
    try {
      await axios.get(`/api/github/restore?id=${gistId}`);
  ...
  const deleteBackup = async (gistId) => {
    try {
      ...
      const response = await axios.delete(`/api/github/delete-backup?id=${gistId}`);
  ...
  const handleQRUpload = async (file) => {
    ...
    try {
      ...
      if (code) {
        const response = await axios.post('/api/totp/import', { qrData: code.data });
